...
from features import build_training_dataset, normalize_zone
...
def train_zone(zone: str) -> None:
    ...
    if df.empty:
        print(f"Sense dades per a la zona {zone}")
        return
    ...
    if data["X_classifier"].empty or data["y_classifier"].empty:
        print(
            f"Sense dades útils dins de la franja intermèdia per classifier a {zone}"
        )
        return
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Entrena el model d'una zona concreta."
    )
    parser.add_argument(
        "--zone",
        required=True,
        help='Zona a entrenar, per exemple: "Bonsai big" o "Bonsai small".',
    )
